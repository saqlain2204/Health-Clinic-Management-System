...
class Department:
    ...
    def add_department(self):
        ...
        self.id = generate_department_id()  # generate unique department ID

        # Check if department ID already exists
        if self.verify_department_id(self.id):
            st.error('Department ID already exists. Please try again with a different ID.')
            return  # Stop further execution if ID already exists

        save = st.button('Save')

        # Execute SQLite statements to save the new department record to the database
        if save:
            conn, c = db.connection()
            with conn:
                c.execute(
            ...
            # After saving, provide an option to add another department
            add_another = st.button('Add Another Department')
            if add_another:
                # Reset the form by clearing the fields
                self.__init__()  # This will reset the instance attributes
                st.experimental_rerun()  # Re-run the script to reset the form

    # method to verify if department ID already exists
    def verify_department_id(self, department_id):
        conn, c = db.connection()
        with conn:
            c.execute(
                """
            ...
            result = c.fetchone()
        conn.close()
        return result is not None  # Return True if ID exists, else False
